...
import { getCars, createCar } from '../servises/api';
...
function renderGarageControls(): string {
  return `<div class="garage__controls">
    ...
    <div class="garage__update-wrap">
      ...
      <input type="color" class="garage__update-color-input" value="#a6ca16">
...
function renderTitleGarage(carsCount: number): string {
  return `<h3 class="h3">Garage <span>(${carsCount})</span></h3>`;
}
...
function renderCars(arrCars: IGetCars[]): HTMLElement {
  ...
  arrCars.forEach((el) => {
    ...
    <div class="garage__per-car-set-wrap">
      <button class="garage__select-btn">Select</button>
      <button class="garage__revove-btn">Remove</button>
      <p class="garage__car-name">${el.name}</p>
    </div>
    <div class="garage__per-car-wrap">
      <button class="garage__start-btn">Start</button>
      <button class="garage__stop-btn">Stop</button>
...
function renderPageGarage(pageGarageCount: number): string {
  ...
  <div class="garage__main-settings-wrap">
      <button class="garage__main-settings-race-btn">prev</button>
      <button class="garage__main-settings-reset-btn">next</button>      
    </div>`;
}
