...
import { getCars, setCar } from '../servises/api';
...
function renderCars(arrCars: IGetCars[]): HTMLElement {
  ...
  arrCars.forEach((el) => {
    ...
    <div class="garage__per-car-set-wrap">
      <button class="garage__select-btn" carID=${el.id}>Select</button>
      <button class="garage__revove-btn" carID=${el.id}>Remove</button>
      <p class="garage__car-name">${el.name}</p>
    </div>
    <div class="garage__per-car-wrap">
      <button class="garage__start-btn" carID=${el.id}>Start</button>
      <button class="garage__stop-btn" carID=${el.id}>Stop</button>
...
window.onload = () => renderGaragePage();
